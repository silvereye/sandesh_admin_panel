  ...
  <body>
    <table class="style1 tablesorter" cellspacing="0">
            ...
            <tbody>
  			...
  			String sql="SELECT * FROM msgs ms, msgrcpt mt where ms.mail_id=mt.mail_id and (( (select email from maddr where id=mt.rid ) like ?) or ( (select email from maddr where id=ms.sid ) like ?)) and ms.content NOT IN (?, ?, ?,?,?,?,?) ORDER BY time_num DESC LIMIT ? , ?";
