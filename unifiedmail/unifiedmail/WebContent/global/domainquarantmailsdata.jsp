  ...
  <body>
    <table class="style1 tablesorter" cellspacing="0">
            ...
            <tbody>
  			...
  			String sql="SELECT * FROM msgs ms, msgrcpt mt where ms.mail_id=mt.mail_id and (( (select email from maddr where id=mt.rid ) like ?) or ( (select email from maddr where id=ms.sid ) like ?)) and ms.quar_type=? ORDER BY time_num DESC LIMIT ? , ?";
  ...
  sid=smail.substring(ind+1,smail.length()-1);
   ...
   if(smail.length()>30)
	...
	rmail=rmail.substring(0,30)+"......";
     ...
     String ctit="Quarantined";
     String mlid=rs.getString("ms.mail_id"); 
     /*
      if(ctype.equalsIgnoreCase("V"))
       {
       ctit="Virus";
       }
        ...
        */        
                 %>
                    <tr> 
                        ...
                        <td class="vcenter" >
                        <a class="tooltips" href="#">Q
<span><%=ctit %></span></a>
                        </td>
                         <td><a style="cursor: pointer;" onclick="qRel('<%=mlid %>','<%=rmail %>')">
Release</a></td>
                    </tr>
