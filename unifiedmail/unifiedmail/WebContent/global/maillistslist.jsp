...
<html>
  <head>
    ...
    <title>Mail List Lists | Silvereye Unified E-mail Server</title>
...
alertify.confirm("Do you want to Delete list: "+val+"<br><br>", function (e) {
				if (e) {
					//alertify.success("You've clicked OK");
					window.location="/unifiedmail/servlet/DelelteMailListServlet?ml="+val;
					//document.getElementById(""+val).href ="/unifiedmail/servlet/DelelteUserServlet?ml="+val;
				} else {
...
/* 	var y=document.f.recordbottum.selectedIndex;
   ...
   document.getElementById("hidc").value = vall; 
    ...
    xmlhttp.open("GET","/unifiedmail/servlet/CountMailListServlet?val="+uid+"&dm="+dm,true);
    // xmlhttp.open("GET","/unifiedmail/servlet/CountMailListServlet?val="+uid+"&dm="+dm,true);
...
for (var i = 0; i < chklength; i++) {  
  ...
  {
  document.f.action = "/unifiedmail/servlet/EditMailListAtionServlet";
   return true;
  }
  }
...
var i=document.f.sortbottum.selectedIndex;
   ...
   document.getElementById('action_gif').style.display= 'none';
    ...
    xmlhttp.open("GET","/unifiedmail/global/maillistdata.jsp?val="+v+"&dm="+dm+"&cnt="+cnt+"&uid="+uid+"&perpg="+perpg,true);
   //var xx=document.f.chk.value;
   //alert(xx);
    xmlhttp.send();
...
<li><a class="active-subs" href="/unifiedmail/global/maillistslist.jsp?dm=<%=dm %>">Mail Lists</a></li>
<li><a href="/unifiedmail/global/mailaliaseslist.jsp?dm=<%=dm %>">Mail Aliases</a></li>
   	...
   	<div class="clr"></div>
        ...
        <%
        int cnt=0;
        cnt=NoOfUser.listCount(url,username,password,base1,dm);
 ...
 <input type="hidden" id="hid_perm" name="hid_perm" value=""/>
  ...
  <input type="hidden" id="hidperpage" name="hidperpage" value="10"/>
    ...
    {  
     ...
     %>
     <div class="main-heading2" id="msg_suc"> Selected mail lists were <%=stt %>. <span class="close">	<a onclick="removeMsg('msg_suc')" title="click me to remove" style="cursor: pointer;">X</a>	</span>	</div>
      <%}
      %>
    
                	
                	<div class="main-heading">	Mail List under domain: <%=dm %>	
                    <div class="row-one">
