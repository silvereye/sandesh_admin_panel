...
public class DelelteUserSearchServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		////delete users	
			String base="ou=Users,domainName="+dm+",o=domains,"+mbase;//dc=springbirds,dc=co,dc=in";//"ou=Users,domainName="+arr_dm[i]+",o=domains"+(String)hs.getAttribute("base");
			   ...
			   /* Date date = new Date();
			    ...
			    Process p = Runtime.getRuntime().exec("sudo mv /maildir/domains/"+dm+"/users/"+nm+" /maildir/domains/"+dm+"/deleted/deleted_"+nm+"" );
		...
		if(page.equalsIgnoreCase("search"))
		{
			...
			response.sendRedirect("/unifiedmail/global/search.jsp?search="+search);
		}
		else if(page.equalsIgnoreCase("advance"))
		{
			String dmm=request.getParameter("dm");
			String fil=request.getParameter("fil");
			String pat=request.getParameter("pat");
			response.sendRedirect("/unifiedmail/global/advanceusersearch.jsp?page=advance&dm="+dmm+"&fil="+fil+"&pat="+pat);
		}
		else
		{
