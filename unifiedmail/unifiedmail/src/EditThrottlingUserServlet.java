...
public class EditThrottlingUserServlet extends HttpServlet {
...
String ml=request.getParameter("hid_mail");
		...
		ps.setInt(1, outpid);
		ps.setString(2, "Sender Quotas for "+ml);
		ps.setString(3, "Sender:user@domain");
		...
		outqid=Crudoperation_Policy.getQtID(outpid,"Sender:user@domain");
		...
		ps.setInt(3,out_max);
		ps.setString(4, "limit to send outgoing mail for "+ml);
		ps.setInt(5, out_dis);
		ps.executeUpdate();
		}
		catch (Exception e) {
		...
		PreparedStatement ps=con.prepareStatement(sql);
		ps.setInt(1, inpid);
		ps.setString(2, "Recipient Quotas for "+ml);
		ps.setString(3, "Recipient:user@domain");
		ps.setInt(4,in_time);
		ps.setString(5, "REJECT");
		ps.setInt(6, in_dis);
		ps.executeUpdate();
		...
		inqid=Crudoperation_Policy.getQtID(inpid,"Recipient:user@domain");
		...
		ps.setString(2, "MessageCount");
		ps.setInt(3,in_max);
		ps.setString(4, "limit to reciept incoming mail for "+ml);
		ps.setInt(5, in_dis);
		ps.executeUpdate();
		}
		catch (Exception e) {
		...
		String data=ip+","+adm+","+dn+",Edit User Profile -Throttling : "+ml;
		int mg=Crudoperation.insertlog(sql, data);
		 ...
		 response.sendRedirect("/unifiedmail/global/throttlinguser.jsp?ml="+ml+"&status=succ");
		out.flush();
		out.close();
	}
